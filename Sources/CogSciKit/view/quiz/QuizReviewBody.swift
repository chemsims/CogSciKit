...
struct QuizReviewBody<QP: QuizPersistence, Analytics: AppAnalytics>: View where QP.QuestionSet == Analytics.QuestionSet {
    ...
    var body: some View {
        ScrollView {
                VStack(spacing: 20) {
                    ...
                    ForEach(model.answeredQuestions, id: \.id) { question in
    ...
    private func reviewCard(
        ...
        let index = model.answeredQuestions.firstIndex(where: { $0.id == question.id }) ?? 0
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            TextLinesView(
                ...
                color: isCorrect ? CorePalette.Quiz.reviewCorrectAnswerFont : CorePalette.Quiz.reviewWrongAnswerFont
            )
            .accessibility(label: Text(label))
