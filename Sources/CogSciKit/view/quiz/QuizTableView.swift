...
private struct QuizTableViewWithGeometry: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ForEach(0..<settings.table.cols, id: \.self) { i in
                col(colIndex: i)
            }
    ...
    private func col(colIndex: Int) -> some View {
        VStack(spacing: 0) {
            ForEach(0..<settings.table.rows.count, id: \.self) { rowIndex in
                cell(
    ...
    private func cell(
        ...
        .background(
            rowIndex % 2 == 0 ? CorePalette.Quiz.tableEvenRow : CorePalette.Quiz.tableOddRow
        )
        .border(CorePalette.Quiz.tableCellBorder, width: 0.5)
