...
struct QuizQuestionsBody<QP: QuizPersistence, Analytics: AppAnalytics>: View where QP.QuestionSet == Analytics.QuestionSet {
    ...
    private var infoIcon: some View {
        ZStack {
            Circle()
                .foregroundColor(CorePalette.Quiz.infoIconBackground)

            Image(systemName: "info.circle")
                .resizable()
                .aspectRatio(contentMode: .fit)
                .foregroundColor(CorePalette.Quiz.infoIconForeground)
        }
...
private struct QuizAnswerOption<QP: QuizPersistence, Analytics: AppAnalytics>: View where QP.QuestionSet == Analytics.QuestionSet {
    ...
    private var answerBackground: Color {
        if styleCorrect {
            return CorePalette.Quiz.correctAnswerBackground
        } else if styleIncorrect {
            return CorePalette.Quiz.wrongAnswerBackground
        } else if model.hasSelectedCorrectOption {
            return CorePalette.Quiz.disabledAnswerBackground
        }
        return CorePalette.Quiz.answerBackground
    }

    private var answerBorder: Color {
        if styleCorrect {
            return CorePalette.Quiz.correctAnswerBorder
        } else if styleIncorrect {
            return CorePalette.Quiz.wrongAnswerBorder
        }
        return CorePalette.Quiz.answerBackground
    }
